...
export function summarizeAttempts(rows) {
  ...
  return {
    ...
    byLanguage: languageBreakdown(practice, questions),
  };
}

function languageBreakdown(practice, questions) {
  const out = {};
  for (const lang of ["sql", "python"]) {
    const qs = questions.filter((q) => q.category === lang);
    out[lang] = {
      runs: practice.filter((r) => (r.questions?.category || r.language) === lang).length,
      solved: qs.filter((q) => q.passed).length,
      firstTryRate: qs.length ? Math.round((qs.filter((q) => q.firstTryPassed).length / qs.length) * 100) : 0,
    };
  }
  return out;
}
...
// Aggregate-only comparison with other users (cohort_skill_benchmark in
// db/app-schema.sql). Throws if the function isn't deployed yet.
export async function loadCohortBenchmark() {
  const { supabase } = await ctx();
  const { data, error } = await supabase.rpc("cohort_skill_benchmark");
  if (error) throw error;
  return data; // { cohort_size, min_cohort, rates: { slug: 0..1 }, beats_pct }
}
